...
def show(parent):
    ...
    # Khung chứa cả frame_info và frame_btn
    top_frame = tk.Frame(win, bg=BG_COLOR)
    top_frame.pack(fill="x", padx=20, pady=15)

    # ===== FRAME THÔNG TIN (LEFT) =====
    ...
    # ------------------- CÁC TRƯỜNG NHẬP LIỆU (9 trường) -------------------
    
    # Danh sách các trường (Label, Row, Col, Type)
    fields = [
        ("Mã NV", 0, 0, 'Entry'), 
        ("Tên NV", 1, 0, 'Entry'), 
        ("Giới tính", 2, 0, 'Combo'), 
    ...
    def load_data():
        ...
        try:
            cur = conn.cursor()
            # Lấy các cột hiển thị (BỎ QUA MẬT KHẨU)
    ...
    def validate_and_get_data(is_update=False):
        """Kiểm tra và lấy dữ liệu từ Entry."""
        data = {
    ...
    def sua_nv():
        ...
        try:
            ...
            if data['MatKhau']:
                # UPDATE cả Mật khẩu nếu người dùng nhập mật khẩu mới
                sql = """UPDATE NHANVIEN
                ...
                # Không UPDATE Mật khẩu nếu ô trống (Giữ lại mật khẩu cũ)
                sql = """UPDATE NHANVIEN
            ...
            load_data() # Tải lại dữ liệu để cập nhật tất cả (an toàn nhất)
            clear_input()
